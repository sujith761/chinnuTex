...
export default function SizingDetailPage() {
  ...
  const yarnImages = {
    ...
    'pc-blended': 'https://images.unsplash.com/photo-1608919291141-a0659c826505?w=600&h=400&fit=crop',
    'pv-blended': 'https://images.unsplash.com/photo-1591195853828-11db59a44f6b?w=600&h=400&fit=crop',
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-red-50 to-pink-100 flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <p className="mb-6 text-gray-600">The sizing product you're looking for doesn't exist.</p>
          <button onClick={() => navigate(-1)} className="px-8 py-3 bg-gradient-to-r from-sky-600 to-blue-600 text-white rounded-xl font-bold hover:shadow-xl transition-all">
            Go Back
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-sky-50 via-blue-50 to-cyan-100">
      ...
      <div className="relative h-96 overflow-hidden">
        <div 
          ...
          style={{ backgroundImage: `url(${yarnImage})` }}
        />
        <div className="absolute inset-0 bg-gradient-to-b from-sky-900/70 via-blue-900/60 to-transparent"></div>
        
        {/* Floating badge */}
        <div className="absolute top-8 right-8 bg-white/90 backdrop-blur-sm px-6 py-3 rounded-2xl shadow-xl">
          <div className="text-xs text-gray-600 mb-1">Sizing Product</div>
          <div className="text-lg font-bold text-sky-600">Premium Quality</div>
        </div>
        ...
        <div className="absolute top-8 left-1/2 transform -translate-x-1/2 w-full max-w-md px-4">
          <div className="relative">
            <input
              type="text"
              placeholder="Search other yarn types..."
              value={searchQuery}
              onChange={(e) => handleSearch(e.target.value)}
              onFocus={() => searchResults.length > 0 && setShowSearch(true)}
              onBlur={() => setTimeout(() => setShowSearch(false), 200)}
              className="w-full px-6 py-3 pl-12 bg-white/90 backdrop-blur-sm border-2 border-sky-200 rounded-2xl text-sm focus:outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-100 transition shadow-lg"
            />
            <svg className="absolute left-4 top-3.5 w-4 h-4 text-sky-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>

            {/* Search Results Dropdown */}
            {showSearch && (searchResults.length > 0 || searchQuery.trim()) && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-white border-2 border-sky-200 rounded-2xl shadow-xl z-50 max-h-80 overflow-y-auto">
                {searchResults.length > 0 ? (
                  <>
                    {searchResults.map((result) => (
                      <div key={result.name} className="border-b border-sky-100">
                        <div className="px-4 py-2 bg-sky-50 text-xs font-bold text-sky-700">
                          {result.name}
                        </div>
                        ...
                        {result.yarn && (
                          <button
                            ...
                            className="w-full text-left px-4 py-2.5 hover:bg-sky-50 border-b border-sky-50 transition flex items-center justify-between group text-sm"
                          >
                            <div>
                            ...
                            <svg className="w-4 h-4 text-sky-400 group-hover:text-sky-600 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        {result.cloth && (
                          <button
                            ...
                            className="w-full text-left px-4 py-2.5 hover:bg-sky-50 transition flex items-center justify-between group text-sm"
                          >
                            <div>
                    ...
                    {/* Navigation Options */}
                    <div>
                      <div className="px-4 py-2 bg-sky-50 text-xs font-bold text-sky-700 uppercase tracking-wider">
                        Browse
                      </div>
                      <button
                        type="button"
                        onMouseDown={() => handleNavigation('/products')}
                        className="w-full text-left px-4 py-3 hover:bg-sky-50 border-b border-sky-100 transition flex items-center justify-between group text-sm"
                      >
                        <div>
                      ...
                      <button
                        ...
                        className="w-full text-left px-4 py-3 hover:bg-sky-50 transition flex items-center justify-between group text-sm"
                      >
                        <div>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-12">
          <div className="container mx-auto">
            <div className="max-w-4xl">
              <p className="text-sm text-sky-200 uppercase tracking-wider mb-3 font-bold">Sizing Product</p>
              <h1 className="text-3xl md:text-4xl font-extrabold text-white drop-shadow-2xl">{yarnName}</h1>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-16">
        <div className="max-w-4xl mx-auto">
          {/* Main card */}
          <div className="bg-white rounded-3xl shadow-2xl overflow-hidden border border-sky-100 -mt-32 relative z-10">
            <div className="p-12">
              {/* Description */}
              <div className="mb-10">
                ...
                <p className="text-gray-700 leading-relaxed text-sm">
                  ...
                  Get optimal results with our specialized sizing formulation designed for superior yarn quality and performance.
                </p>
              </div>

              {/* Price Section */}
              <div className="bg-gradient-to-r from-sky-50 to-blue-50 rounded-2xl p-8 mb-10">
                <h3 className="text-lg font-bold text-gray-800 mb-4">Pricing Information</h3>
                <div className="flex items-baseline gap-3 mb-4">
                  <span className="text-3xl font-extrabold text-sky-600">₹{yarnPricePerKg}</span>
                ...
                <div className="bg-white/60 rounded-xl p-4 mb-4">
                  ...
                  <ul className="text-sm text-gray-600 mt-2 space-y-1">
                    <li>• 10 kg: ₹{yarnPricePerKg * 10}</li>
                    <li>• 50 kg: ₹{yarnPricePerKg * 50}</li>
                    <li>• 100 kg: ₹{yarnPricePerKg * 100}</li>
                  </ul>
                </div>
                <p className="text-sm text-gray-500">Bulk discounts available upon request</p>
              </div>

              {/* Features */}
              <div className="mb-10">
                <h3 className="text-lg font-bold text-gray-800 mb-6">Key Benefits</h3>
                <div className="grid md:grid-cols-2 gap-6">
                  {[
                    'Enhanced yarn strength',
                    'Improved abrasion resistance',
                    'Better weaving performance',
                    'Superior yarn smoothness',
                    'Consistent quality',
              ...
              <div className="border-t border-gray-200 pt-10">
                ...
                <p className="text-gray-700 text-sm mb-8">Book your {yarnName.toLowerCase()} sizing product today and experience superior yarn quality.</p>
                ...
                <Link 
                  ...
                  className="inline-flex items-center gap-2 bg-gradient-to-r from-sky-600 to-blue-600 text-white px-10 py-4 rounded-xl font-bold hover:from-sky-700 hover:to-blue-700 transition-all shadow-xl hover:shadow-2xl text-lg"
                >
